...
make_instr_func(sub_r2rm_b)
{
    ...
    rm.val = alu_sub(sign_ext(r.val & (0xFFFFFFFF >> k), data_size), sign_ext(rm.val & (0xFFFFFFFF >> k), data_size), data_size);
...
make_instr_func(sub_rm2r_b)
{
    ...
    operand_read(&r);
    uint8_t k = 32 - data_size;
    r.val = alu_sub(sign_ext(rm.val & (0xFFFFFFFF >> k), data_size), sign_ext(r.val & (0xFFFFFFFF >> k), data_size), data_size);
...
make_instr_func(sub_i2a_b)
{
    ...
    operand_read(&imm);
    uint8_t k = 32 - data_size;
    r.val = alu_sub(sign_ext(imm.val & (0xFFFFFFFF >> k), data_size), sign_ext(r.val & (0xFFFFFFFF >> k), data_size), data_size);
    operand_write(&r);
